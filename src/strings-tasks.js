...
function findLongestWord(sentence) {
  const newStrin = sentence.split(' ');
  let word = '';
  for (let i = 0; i < newStrin.length; i += 1) {
    if (newStrin[i].length > word.length) {
      word = newStrin[i];
    }
  }
  return word;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < splitStr.length; i += 1) {
    newString += `${splitStr[i].split('').reverse().join('')} `;
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 90) {
      newChar = ((str.charCodeAt(i) - 65 + 13) % 26) + 65;
    } else if (str.charCodeAt(i) >= 97 && str.charCodeAt(i) <= 122) {
      newChar = ((str.charCodeAt(i) - 97 + 13) % 26) + 97;
    } else newChar = str.charCodeAt(i);
...
function getCardId(value) {
  ...
  let result = 0;
  for (let i = 0; i < valueCards.length; i += 1) {
    if (value === valueCards[i]) {
      result = i;
    }
  }
  return result;
